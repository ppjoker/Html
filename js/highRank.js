var tab = document.getElementById("high");
			
			var db=openDatabase('mybd','1.0','text db','1024*1024');
			
			var user = new Array();
			db.transaction(function(contex){
				contex.executeSql('select * from highRank',[],function(con,data){
					...
					for(var i=0;i<rows;i++){
						...
						obj = {name:data.rows.item(i).name,timer:data.rows.item(i).timer,score:data.rows.item(i).score};
						user.push(obj);
					...
					str += '<TR><TD>'+user[i].name+'</TD><TD>'+user[i].score+'条</TD><TD>'+user[i].timer+'秒</TD></TR>';
					}
					tab.innerHTML = str;
					//tab.innerHTML = str;
				});
